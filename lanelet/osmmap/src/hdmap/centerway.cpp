...
void CenterWay::FindNearestPoint(const std::array<double, 3> &cur, std::array<double, 3> &tar) const
{
    ...
    for(auto it = Begin(); it != End(); ++it)
    {
        for(int i = 0; i < it->second->length_ - 1; ++i)
        {
            int tarpointid = it->second->centernodeline_[i];
            //路方向向量
            double bx = centerpointmap_.at(tarpointid+1)->x_ - centerpointmap_.at(tarpointid)->x_;
            double by = centerpointmap_.at(tarpointid+1)->y_ - centerpointmap_.at(tarpointid)->y_;
            if(xx * bx + yy * by > 0)//车路同向<90度
            {
                double d = distance2d(cur[0], cur[1], centerpointmap_.at(tarpointid)->x_, centerpointmap_.at(tarpointid)->y_);
                if(d > 3.0 && d < distemp)
                {
                    tar[0] = centerpointmap_.at(tarpointid)->x_;
                    tar[1] = centerpointmap_.at(tarpointid)->y_;
                    tar[2] = std::atan2(by, bx);
                    distemp = d;
                }
            }
        }
