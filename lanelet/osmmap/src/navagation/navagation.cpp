...
NavagationBase::NavagationBase(ros::NodeHandle &n):n_(n)
{
    ...
    //返回地图目标点服务
    gettargetpoint_server_ = n_.advertiseService("gettargetpoint_service", &NavagationBase::GetTargetPointService, this);
    getneighborlanelet_server_ = n_.advertiseService("getneighborlanelet_service", &NavagationBase::GetNeighborLaneletService, this);
    getsamedirectionlanelet_server_ = n_.advertiseService("getsamedirectionlanelet_service", &NavagationBase::GetSameDirectionLaneletService, this);
}
...
void NavagationBase::SmoothPath()
{
    ...
    // if(smoothpathnode_.size() < 2) return;
    // std::vector<double> tempx, tempy, tempz;
    // for(int i = 0; i < smoothpathnode_.size(); ++i)
    // {
    //     tempx.push_back(smoothpathnode_[i].x_);
    //     tempy.push_back(smoothpathnode_[i].y_);
    //     tempz.push_back(smoothpathnode_[i].ele_);
    // }
    // plan::FemPosDeviationOsqp smooth(tempx, tempy, tempz);
    // smooth.SetWeightRef({1.0, 1.0, 1.0});
    // tempx.clear();
    // tempy.clear();
    // tempz.clear();
    // smooth.Solve(&tempx, &tempy, &tempz);
    // std::vector<map::centerway::CenterPoint3D> temppathnode;
    // for(int i = 0; i < tempx.size(); ++i)
    // {
    //     map::centerway::CenterPoint3D pointtemp;
...
bool NavagationBase::GetTargetPointService(osmmap::GetTargetPoint::Request &request, osmmap::GetTargetPoint::Response &response)
{
    auto distance2d = [](double x0, double y0, double x1, double y1)
    {
        return std::sqrt((x0 - x1) * (x0 - x1) + (y0 - y1) * (y0 - y1));
    };
    if(distance2d(request.Sx, request.Sy, end_state_[0], end_state_[1]) < 10.0)
    {
        response.Tx = end_state_[0];
        response.Ty = end_state_[1];
        response.Tyaw = end_state_[2];//返回终点
        return true;//(TODO)这里如果返回false，会出现问题，原因不明
    }
    std::array<double, 3> cur{request.Sx, request.Sy, request.Syaw};
    std::array<double, 3> tar;
    centerwaysptr_->FindNearestPoint(cur, tar);
    response.Tx = tar[0];
    response.Ty = tar[1];
    response.Tyaw = tar[2];
    return true;
...
bool NavagationBase::GetNeighborLaneletService(osmmap::GetNeighborLanelet::Request &request, osmmap::GetNeighborLanelet::Response &response)
{
    map::centerway::CenterPoint3D reqpoint(request.Sx, request.Sy);
    int reqpoint_res = globalplans_->InWhichCenterway(reqpoint, nodesptr_, waysptr_, relationsptr_);
    if(reqpoint_res == -1) return false;//当前点在车道之外
    map::centerway::CenterWay3D *reqway = centerwaysptr_->Find(reqpoint_res);
    if(reqway->neighbours_.first != -1)
    {
        //左
        map::centerway::CenterWay3D *centerline = centerwaysptr_->Find(reqway->neighbours_.first);
        int i;
        for(i = 0; i < centerline->length_ - 1; ++i)
        {
            double pathx = centerwaysptr_->FindCenterPoint(centerline->centernodeline_[i+1])->x_ - centerwaysptr_->FindCenterPoint(centerline->centernodeline_[i])->x_;
            double pathy = centerwaysptr_->FindCenterPoint(centerline->centernodeline_[i+1])->y_ - centerwaysptr_->FindCenterPoint(centerline->centernodeline_[i])->y_;
            double carx = centerwaysptr_->FindCenterPoint(centerline->centernodeline_[i+1])->x_ - request.Sx;
            double cary = centerwaysptr_->FindCenterPoint(centerline->centernodeline_[i+1])->y_ - request.Sy;

            // if(carx * pathx + cary * pathy > 0) return centerline->centernodeline_[i+1];//给出导航路线包括车辆本身
            if(carx * pathx + cary * pathy > 0) break;//给出纯参考线
        }
        for(; i < centerline->length_; ++i)
        {
            geometry_msgs::Point p;
            p.x = centerwaysptr_->FindCenterPoint(centerline->centernodeline_[i])->x_;
            p.y = centerwaysptr_->FindCenterPoint(centerline->centernodeline_[i])->y_;
            p.z = centerwaysptr_->FindCenterPoint(centerline->centernodeline_[i])->ele_;
            response.Left_path.points.push_back(p);
        }
    }
    if(reqway->neighbours_.second != -1)
    {
        //右
        map::centerway::CenterWay3D *centerline = centerwaysptr_->Find(reqway->neighbours_.second);
        int i;
        for(i = 0; i < centerline->length_ - 1; ++i)
        {
            double pathx = centerwaysptr_->FindCenterPoint(centerline->centernodeline_[i+1])->x_ - centerwaysptr_->FindCenterPoint(centerline->centernodeline_[i])->x_;
        ...
        for(; i < centerline->length_; ++i)
        {
            ...
            p.y = centerwaysptr_->FindCenterPoint(centerline->centernodeline_[i])->y_;
            p.z = centerwaysptr_->FindCenterPoint(centerline->centernodeline_[i])->ele_;
            response.Right_path.points.push_back(p);
        }
    }
    return true;
}

bool NavagationBase::GetSameDirectionLaneletService(osmmap::GetSameDirectionLanelet::Request &request, osmmap::GetSameDirectionLanelet::Response &response)
{
    map::centerway::CenterPoint3D reqpoint(request.Sx, request.Sy);
    int reqpoint_res = globalplans_->InWhichCenterway(reqpoint, nodesptr_, waysptr_, relationsptr_);
    if(reqpoint_res == -1) return false;//当前点在车道之外
    std::vector<int> candidates = globalplans_->FindNextLanes(reqpoint_res);
    // centerwaysptr_->FindNeighbor(reqpoint_res, candidates);
    for(int i = 0; i < candidates.size(); ++i)
    {
        map::relation::relationship *lanetmp = relationsptr_->Find(candidates[i]);
        visualization_msgs::Marker lanelet_marker;
        map::way::Line *leftline = waysptr_->Find(lanetmp->leftedge_.ID_);
        map::way::Line *rightline = waysptr_->Find(lanetmp->rightedge_.ID_);
        //左边界
        for(int j = 0; j < leftline->length_; ++j)
        {
            geometry_msgs::Point p;
            p.x = nodesptr_->Find(leftline->nodeline_[j])->local_x_;
            p.y = nodesptr_->Find(leftline->nodeline_[j])->local_y_;
            p.z = nodesptr_->Find(leftline->nodeline_[j])->elevation_;
            lanelet_marker.points.push_back(p);
        }
        //右边界
        for(int j = rightline->length_ - 1; j >= 0; --j)
        {
            geometry_msgs::Point p;
            p.x = nodesptr_->Find(rightline->nodeline_[j])->local_x_;
            p.y = nodesptr_->Find(rightline->nodeline_[j])->local_y_;
            p.z = nodesptr_->Find(rightline->nodeline_[j])->elevation_;
            lanelet_marker.points.push_back(p);
        }
        response.lanelets.push_back(lanelet_marker);
    }
    return true;
}
