 ...
 * @LastEditors: wpbit
 * @LastEditTime: 2023-02-16 15:59:38
 * @Description: 
 */
...
{
NavagationPure::NavagationPure(ros::NodeHandle &n) : NavagationBase(n)
{
    car_model_pub_ = n.advertise<visualization_msgs::Marker>("/car_model", 1);
    gps_sub_ = n_.subscribe("/carla/ego_vehicle/odometry", 1, &NavagationPure::PureCallback, this);
}

void NavagationPure::PureCallback(const nav_msgs::Odometry::ConstPtr &msg)
{
    //起点位姿
    Eigen::Quaterniond q_start = Eigen::Quaterniond(0.609886348247528, 0.0135536137968302, 0.00340890442021191, -0.792365729808807).normalized();
    
    //坐标变换
    atnowpoint_->local_x_ = msg->pose.pose.position.x;
    atnowpoint_->local_y_ = msg->pose.pose.position.y;
    atnowpoint_->elevation_ = msg->pose.pose.position.z-1.5;
    ...
    Eigen::Quaterniond q = (q_start.inverse() * q1).normalized();
    qenu2base.setW(q.w());
    qenu2base.setX(q.x());
    qenu2base.setY(q.y());
    qenu2base.setZ(q.z());
    ...
    baselink2map_.setOrigin(tf::Vector3(atnowpoint_->local_x_, atnowpoint_->local_y_, atnowpoint_->elevation_));
    broadcaster_.sendTransform(tf::StampedTransform(baselink2map_, ros::Time::now(), "map", "base_link"));
    ...
    //发布车模型
    visualization_msgs::Marker carmaodel;
    carmaodel.id = 0;
    carmaodel.header.frame_id = "base_link";
    carmaodel.header.stamp = msg->header.stamp;
    carmaodel.ns = "head_car";
    carmaodel.action = visualization_msgs::Marker::ADD;
    carmaodel.type = visualization_msgs::Marker::CUBE;
    carmaodel.color.r = 0.0;
    carmaodel.color.g = 1.0;
    carmaodel.color.b = 0.0;
    carmaodel.color.a = 0.3;
    carmaodel.scale.x = 17.155;
    carmaodel.scale.y = 3.08;
    carmaodel.scale.z = 2.5;
    carmaodel.pose.position.x = 0.0;
    carmaodel.pose.position.y = 0.0;
    carmaodel.pose.position.z = 0.0;
    carmaodel.pose.orientation.w = 1.0;
    //car_model_pub_.publish(carmaodel);
}

void NavagationPure::Process()
{
    ROS_INFO("\033[1;32m --> PURE mode is working ...  \033[0m\n");
    //main loop
    ros::Rate r(10);
    while(n_.ok())
    {
