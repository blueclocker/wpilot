 ...
 * @Date: 2022-09-16 13:58:38
 * @LastEditTime: 2022-09-16 19:31:27
 * @LastEditors: blueclocker 1456055290@hnu.edu.cn
 * @Description: 
 * @FilePath: /catkin_ws/src/rrtstar/src/rrt_flow.cpp
...
namespace RRT
{
RRTflow::RRTflow(ros::NodeHandle &nh)
{
    rrt_plan_ = new RRT();
    start_point_sub_ = nh.subscribe("/initialpose", 1, &RRTflow::startPointCallBack, this);
    goal_point_sub_ = nh.subscribe("/move_base_simple/goal", 1, &RRTflow::goalPointCallBack, this);
    map_sub_ = nh.subscribe("/map", 1, &RRTflow::mapCallBack, this);
...
void RRTflow::startPointCallBack(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &msg)
{
...
void RRTflow::goalPointCallBack(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    is_plan_ = true;
...
void RRTflow::mapCallBack(const nav_msgs::OccupancyGrid::ConstPtr &msg)
{
    if(has_map_) return;
    map_valid_ = true;
    has_map_ = true;
...
void RRTflow::visualizationPath(const std::vector<vector2d> &path)
{
...
void RRTflow::visualizationNodes(const std::vector<node2d*> &nodes)
{
    visualization_msgs::MarkerArray markerarray;
    visualization_msgs::Marker maker;
    maker.header.frame_id = "map";
