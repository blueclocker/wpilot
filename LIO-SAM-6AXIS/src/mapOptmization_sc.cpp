...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  bool saveMapService(lio_sam_6axis::save_mapRequest &req, lio_sam_6axis::save_mapResponse &res)
  {
    ...
    for (int i = 0; i < (int)cloudKeyPoses3D->size(); i++)
    {
      ...
      //   inter >> s_temp;
      //   // pcl::io::savePCDFileBinary(saveMapDirectory + "CornerMap/" + s_temp + ".pcd", *cornerCloudKeyFrames[i]);
      //   // pcl::io::savePCDFileBinary(saveMapDirectory + "SurfMap/" + s_temp + ".pcd", *surfCloudKeyFrames[i]);
      //   pcl::PointCloud<PointType>::Ptr each_map(new pcl::PointCloud<PointType>);
      //   each_map = rawCloudKeyFrames[i];
      //   pcl::io::savePCDFileBinary(saveMapDirectory + "/RawSubMap/" + s_temp + ".pcd", *each_map);
      //   *globalCornerCloud += *transformPointCloud(cornerCloudKeyFrames[i], &cloudKeyPoses6D->points[i]);
      //   *globalSurfCloud += *transformPointCloud(surfCloudKeyFrames[i], &cloudKeyPoses6D->points[i]);
      *globalMapCloud += *transformPointCloud(rawCloudKeyFrames[i], &cloudKeyPoses6D->points[i]);
...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  void publishOdometry()
  {
    ...
    std::cout << "*****************************************" << std::endl;
    ROS_INFO("X Y Z: %f  %f  %f", transformTobeMapped[3], transformTobeMapped[4], transformTobeMapped[5]);
    ROS_INFO("roll pitch yaw: %f  %f  %f", transformTobeMapped[0], transformTobeMapped[1], transformTobeMapped[2]);
    std::cout << "*****************************************" << std::endl
              << std::endl;
  ...
  void printTxt(string &directory)
  {
    ofstream outfile;
    outfile.open(directory + "/pose.txt");
