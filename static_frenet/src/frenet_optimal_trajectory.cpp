    ...
    { // MAXT  5.0   MINT  4.0   DT  0.2
           ...
           tv += D_T_S)
      {
        ...
        for (float t_ : fp.t)
        {
          ...
          if (fp_bot.s_dd.back() <= fp_bot.max_accel_d) fp_bot.max_accel_d = fp_bot.s_dd.back();
          if (fp_bot.d.back() > fp_bot.max_left_d)
          {
            fp_bot.max_left_d = fp_bot.d.back();
          };
          if (fp_bot.d.back() <= fp_bot.max_right_d) fp_bot.max_right_d = fp_bot.d.back();
    ...
    float c_speed, float c_acc, float c_d, float c_d_d, float c_d_dd, float s0, float frenet_s_goal, float frenet_d_goal)
{
  ...
  { // MAXT  5.0   MINT  4.0   DT  0.2
    ...
    for (float di = -(0.5 * MAX_ROAD_WIDTH - 0.5 * veh_width); di < 0.5 * MAX_ROAD_WIDTH - 0.5 * veh_width; di += D_ROAD_W){
      ...
      fp_bot.cd = KJ * Jp - KT * Ti;
      fp_bot.cv = KJ * Js - KT * Ti + KT * fp_bot.max_accel;//KT
      fp_bot.cf = KLAT * fp_bot.cd + KLON * fp_bot.cv;
      fp_list.push_back(fp_bot);
    } 
  }
  ...
  { 
    ...
    { 
      ...
      { 
        ...
        if(std::fabs(yaw_dis)>Pi) yaw_dis = 2*Pi-std::fabs(path_p->yaw[i + 1])-std::fabs(path_p->yaw[i]);
...
void rPathCallback(const visualization_msgs::Marker::ConstPtr &path_data){
  ...
  // Eigen::Vector2f v1(wx[0]+(float)cosy_cosp, wy[0]+(float)siny_cosp);
  // Eigen::Vector2f v2(wx[1]-wx[0],wy[1]-wy[0]);
  ...
  Vector2D self(cosy_cosp, siny_cosp);
  Vector2D lane(wx[1]-wx[0],wy[1]-wy[0]);
  float cos = ((self*lane).x+(self*lane).y)/(self.length()*lane.length());
  std::cout << "yaw = " << yaw <<" "<<cos<<" cos_self "<<cosy_cosp<< std::endl;//E +-0  W +-180;  N + S-;  now we use velocity-> to represent heading else use this;
  if(cos<0){
    reverse = 1;
  }
  //意图代号，0-巡航，1-停车，2-跟车
  ...
  if(target_speed>15/3.6){
    ...
    if(target_speed<10/3.6)target_speed=10/3.6;
  }
  ...
  else if (action == 1&&(frenet_goal_point[0]>frenet_c_point[0]))
  {
    ...
    // Vec_Path fp_list = calc_frenet_paths_stopping(c_speed,c_acc,c_d, c_d_d,c_d_dd, s0,frenet_goal_point[0], frenet_goal_point[1]);
  ...
  if (final_path.s.empty() && last_path.s.size() > 2 && (count_last_path < 3)&&(frenet_goal_point[0]>frenet_c_point[0]))
  {
    ...
    else
    {
      ...
      alter_final_path = frenet_optimal_planning_stopping(target_speed, csp_obj, s0, c_speed, c_acc, c_d, c_d_d,
                                                          c_d_dd, last_path.s.back(), last_path.d.back(), last_path.yaw.back(),
                                                          obstcles_list, velocity_list);
  ...
  { 
    ...
    alter_final_path = frenet_optimal_planning_stopping(target_speed, csp_obj, s0, c_speed, c_acc, c_d, c_d_d,
                                                        c_d_dd, last_path.s.back(),last_path.d.back(), frenet_goal_point[2], obstcles_list,velocity_list);
  ...
  if(n>0)//1?
  {
    ...
    { 
      ...
      p.z = -2;
      // if(reverse){
      //   if(final_path.s_d[j]<=1.1)
      //   track.v.push_back(-final_path.s_d[j]);
      //   else
      //   track.v.push_back(-1.1);
      // }else{
        // if(final_path.s_d[j]<=1.1)
        track.v.push_back(final_path.s_d[j]);
        // else
        // track.v.push_back(1.1);
      // }
      // track.c.push_back(final_path.c[j]);
      track.w.push_back(final_path.s_d[j]*final_path.c[j]);////
      // track.w.push_back((final_path.yaw[j]-final_path.yaw[0])/final_path.t[j]);
      marker.points.push_back(p);
      std::cout<<"v"<<"[ "<<j<<"] "<<track.v[j]<<"  y"<<"[ "<<j<<"] "<<p.y<<std::endl;
    }
    path_array.markers.clear();
    path_array.markers.push_back(marker);

    track.track_w = track.w[n/2];//output
    track.track_a = final_path.s_dd[n/2];//output

    vehicle_data.target_wheel_angle = track.w[n/2];
    vehicle_data.target_wheel_angular_rate = abs(track.w[n/2])/10;
    if(final_path.s_dd[n/2] > 0)
    {
      vehicle_data.acceleration_pct = final_path.s_dd[n/2]/10;
      vehicle_data.braking_pct = 0;
    }else{
      vehicle_data.braking_pct = -final_path.s_dd[n/2]/10;
      vehicle_data.acceleration_pct = 0;
    }
  }
  path_pub.publish(path_array);
  track_pub.publish(track);
  // lgsvl_pub.publish(vehicle_data);
  time_t finish = clock();
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber r_path_sub = nh.subscribe<visualization_msgs::Marker>("/navagation_node/golbalpath_info", 1, rPathCallback);
  // message_filters::Subscriber<rs_perception::PerceptionListMsg> obs_sub(nh, "/adaptive_clustering/rs_percept_result", 1);
  // message_filters::Subscriber<visualization_msgs::Marker> r_path_sub(nh, "/navagation_node/golbalpath_info", 1);
  // message_filters::Subscriber<osmmap::carState>carstate_sub(nh, "/navagation_node/carstate_info", 1);
  ...
  while(ros::ok()){
    ...
    lgsvl_pub = nh.advertise<lgsvl_msgs::VehicleControlData>("/vehicle_cmd", 1);
    ros::spinOnce();
    r.sleep();
  }
  // ros::spin();
